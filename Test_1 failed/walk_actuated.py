"""
Walking robot with proper actuator control.
Uses IK to compute joint targets, then actuators to reach them.
"""
import numpy as np
import mujoco
...
def compute_ik(model, data, target_foot1, target_foot2, max_iterations=50):
    """Compute joint angles to reach target feet positions"""
    learning_rate = 0.2
...
print("=" * 70)
print("WALKING ROBOT WITH ACTUATOR CONTROL")
print("=" * 70)
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    step_idx = 0
    frame = 0
    total_steps = num_steps * 5  # 5 gait cycles
...
print(f"\n{'='*70}")
print(f"Simulation complete: {frame} frames | {frame/model.opt.timestep:.1f} seconds")
print(f"{'='*70}")

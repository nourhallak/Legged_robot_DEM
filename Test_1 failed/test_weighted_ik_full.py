#!/usr/bin/env python3
"""IK with stance-foot priority weighting"""
import mujoco
import numpy as np
...
def compute_ik_weighted(model, data, base_target, com_target, foot1_target, foot2_target, max_iterations=50, tolerance=0.002):
    """IK with weighted errors - priority to feet on the ground"""
    ...
    # Determine if feet should be in stance or swing based on target height
    # Stance at ~0.21m, Swing at ~0.225m
    f1_in_stance = foot1_target[2] < 0.218  # Below 218mm = stance
    f2_in_stance = foot2_target[2] < 0.218  # Below 218mm = stance
    ...
    for iteration in range(max_iterations):
        ...
        # Weight errors: stance feet get higher priority (weight=2.0), swing feet get lower (weight=0.5)
        # COM and base get default weight (1.0)
        w_base = 1.0
        w_com = 1.0
        w_f1 = 2.0 if f1_in_stance else 0.5
        w_f2 = 2.0 if f2_in_stance else 0.5
...
print("=== Testing Weighted IK (Stance Priority) ===\n")
...
for step_index in range(len(base_traj)):
    ...
    qpos_solution, success = compute_ik_weighted(
        model, data,
    ...
    # Count flying feet (> 10mm above target)
    if abs(f1_err) > 10:
        flying_foot_count += 1
    if abs(f2_err) > 10:
        flying_foot_count += 1
...
print(f"Expected: 0 (all feet should be within 10mm of target)")

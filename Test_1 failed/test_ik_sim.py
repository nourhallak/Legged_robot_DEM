#!/usr/bin/env python3
"""Test: Run IK like ik_simulation.py for first 10 steps"""
import mujoco
import numpy as np
...
print("=== Testing IK like ik_simulation.py ===\n")
print("Step | Base Z | Foot1 Z | Foot2 Z | Status")
print("-" * 55)
...
for step_index in range(10):
    ...
    f1_ok = "✓" if abs(f1_z - target_f1) < 10 else "✗"
    f2_ok = "✓" if abs(f2_z - target_f2) < 10 else "✗"
    
    print(f"{step_index:3d} | {smoothed_qpos[2]:.4f} | {f1_z:7.1f} | {f2_z:7.1f} | {f1_ok} {f2_ok}")
    
    # Save for next iteration
    prev_qpos = smoothed_qpos.copy()

print("\nExpected foot Z: 210-225mm (ground at 210mm + 15mm clearance)")

#!/usr/bin/env python3
"""
Quick test: Run IK solver and check if feet actually match ground contact.
"""
import mujoco
import numpy as np
import os
...
print("\n=== SIMULATION GROUND CONTACT TEST ===\n")
print("Step | Target Foot1 Z | Actual Foot1 Z | Target Foot2 Z | Actual Foot2 Z | Contact OK?")
print("-" * 95)
...
for step in [0, 50, 100, 150, 200, 250, 300, 350, 399]:
    ...
    qpos, success = compute_ik_solution(model, data, base_target, com_target, foot1_target, foot2_target)
    ...
    ground_z = 0.21
    contact_ok_1 = actual_foot1_z >= ground_z
    contact_ok_2 = actual_foot2_z >= ground_z
    contact_ok = contact_ok_1 and contact_ok_2
    
    status = "✓" if contact_ok else "✗"
    
    print(f"{step:3d}  | {target_foot1_z:.6f}       | {actual_foot1_z:.6f}       | {target_foot2_z:.6f}       | {actual_foot2_z:.6f}       | {status}")
    
    if not contact_ok:
        contact_errors.append((step, actual_foot1_z, actual_foot2_z))

print("\n=== RESULT ===")
if not contact_errors:
    print("✓ SUCCESS: All feet maintain ground contact during simulation")
else:
    print(f"✗ ISSUES FOUND at {len(contact_errors)} step(s):")
    for step, z1, z2 in contact_errors:
        print(f"  Step {step}: Foot1 Z={z1:.6f}, Foot2 Z={z2:.6f} (ground at 0.21m)")

#!/usr/bin/env python3
"""
Generate asymmetric gait: Swing phase longer and stronger than stance phase
Creates forward momentum through asymmetric leg motion
"""
...
for i, t in enumerate(t_norm):
    if t < 0.33:
        # Phase 1 (0-0.33): Right leg backward push, Left leg starting swing
        phase = t / 0.33
        ...
        # Phase 2 (0.33-0.67): Right leg return, Left leg SWING (longer, stronger)
        phase = (t - 0.33) / 0.34
        
        right_hip[i] = 0.40 * np.cos(phase * np.pi)  # Returning: 0.40->0
        right_knee[i] = -0.10 * np.sin(phase * np.pi)**2
        right_ankle[i] = 0.06 * np.cos(phase * np.pi)
        
        left_hip[i] = -0.45 * np.sin(phase * np.pi)  # Swing forward: 0->-0.45->0 (STRONG)
        left_knee[i] = -0.50 * np.sin(phase * np.pi)  # High knee
        left_ankle[i] = 0.18 * np.sin(phase * np.pi)
        
    else:
        # Phase 3 (0.67-1.0): Left leg backward push, Right leg starting swing
        phase = (t - 0.67) / 0.33
...
print("[+] Generated ASYMMETRIC FORWARD GAIT")
...
print(f"    Asymmetry: Swing phase is 2.7s longer for momentum accumulation")
print(f"\nGait Phases:")
print(f"    0.00-0.33s: Right leg PUSH back (0.40 rad)")
print(f"    0.33-0.67s: Right leg returns, Left leg SWING forward (-0.45 rad)")
print(f"    0.67-1.00s: Left leg PUSH back (0.40 rad), Right leg at rest")
print(f"    Result: Longer swing phase = net forward momentum")
...
axes[0].set_title('Asymmetric Forward Gait - Longer Swing Phase', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig('asymmetric_forward_gait.png', dpi=100, bbox_inches='tight')
print("\n[+] Saved gait visualization to asymmetric_forward_gait.png")
plt.close()

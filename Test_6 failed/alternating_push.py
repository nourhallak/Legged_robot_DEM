#!/usr/bin/env python3
"""
Alternating push gait - both legs push equally, perfectly 180° out of phase
"""
import mujoco as mj
import numpy as np
...
print("\n" + "=" * 90)
print(" " * 20 + "ALTERNATING PUSH GAIT - PERFECTLY SYMMETRIC")
print("=" * 90)
...
def get_push_phase(t, phase_offset=0):
    """
    Generate asymmetric push: rapid stance (push), slow swing (recover)
    Both legs use identical profile but offset by 180°
    PUSH FORWARD with POSITIVE hip angle (rotate hip forward)
    """
    t_phase = ((t + phase_offset) % gait_period) / gait_period
    
    # RAPID STANCE (0-0.4): Strong push forward (positive hip)
    if t_phase < 0.4:
        progress = t_phase / 0.4
        # Sharp push forward with positive hip angle
        hip_angle = 0.9 * np.sin(progress * np.pi)
        knee_angle = -0.3 - 0.25 * np.sin(progress * np.pi)
        ankle_angle = 0.25
        
    # SLOW SWING (0.4-1.0): Long recovery to prepare for next push
    else:
        progress = (t_phase - 0.4) / 0.6
        # Swing leg back (negative hip)
        hip_angle = 0.9 - 1.8 * (progress * progress)  # Parabolic ease-out
...
print(f"[+] Gait period: {gait_period}s, Rapid stance + slow swing")

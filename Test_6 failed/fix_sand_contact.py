#!/usr/bin/env python3
"""
Generate corrected top-surface sand configuration:
- Robot hip lowered so feet TOUCH sand
...
# Robot hip height - ADJUSTED to make feet touch sand
# Sand top is at 0.435m, feet need to touch around 0.435-0.440m
# Current setup has feet 0.01-0.04m above hip
# So hip should be around 0.395-0.405m to make feet touch at 0.405-0.445m
robot_hip_z = 0.405  # LOWERED from 0.52m to 0.405m

# Sand bed extent
sand_x_min = 0.150  # Sand bed start
sand_x_max = 0.450  # Sand bed end
...
z_layers = [
    floor_z + sand_spacing + 0.0,     # Layer 1: Z=0.426m
    floor_z + sand_spacing * 2,        # Layer 2: Z=0.432m
    floor_z + sand_spacing * 3,        # Layer 3: Z=0.438m (top)
]

print(f"Generating CORRECTED top-surface sand configuration:")
...
print(f"  Robot hip: Z={robot_hip_z}m (LOWERED for sand contact)")
print(f"  Expected foot height: ~{robot_hip_z + 0.03:.3f}m (should touch sand at 0.438m)")
...
# Add robot with CORRECTED hip position
xml_lines.extend([
...
print(f"Robot hip now at Z={robot_hip_z}m (lowered to touch sand)")
